...
static void fused_step_simd(bocpd_asm_t *b, double x)
{
    ...
    for (int j = 0; j < 4; j++)
    {
        if (max_arr_a[j] > map_val) { map_val = max_arr_a[j]; map_idx = idx_arr_a[j]; }
        if (max_arr_b[j] > map_val) { map_val = max_arr_b[j]; map_idx = idx_arr_b[j]; }
    }
                     ...
                     bocpd_prior_t prior, size_t max_run_length)
{
     ...
     *
     *   lin_interleaved:  (cap + 32) * 4 doubles  (for Block B safety)
     *   ss_n:             cap doubles
     *   ss_sum:           cap doubles
     *   ss_sum2:          cap doubles
     *   post_kappa:       cap doubles
     *   post_mu:          cap doubles
     *   post_alpha:       cap doubles
     *   post_beta:        cap doubles
     *   C1:               cap doubles
     *   C2:               cap doubles
     *   sigma_sq:         cap doubles
     *   inv_sigma_sq_nu:  cap doubles
     *   lgamma_alpha:     cap doubles
     *   lgamma_alpha_p5:  cap doubles
     *   r:                (cap + 32) doubles
    ...
    b->lin_interleaved = (double*)ptr;  ptr += bytes_interleaved;
    b->ss_n            = (double*)ptr;  ptr += bytes_vec;
    b->ss_sum          = (double*)ptr;  ptr += bytes_vec;
    b->ss_sum2         = (double*)ptr;  ptr += bytes_vec;
    b->post_kappa      = (double*)ptr;  ptr += bytes_vec;
    b->post_mu         = (double*)ptr;  ptr += bytes_vec;
    b->post_alpha      = (double*)ptr;  ptr += bytes_vec;
    b->post_beta       = (double*)ptr;  ptr += bytes_vec;
    b->C1              = (double*)ptr;  ptr += bytes_vec;
    b->C2              = (double*)ptr;  ptr += bytes_vec;
    b->sigma_sq        = (double*)ptr;  ptr += bytes_vec;
    b->inv_sigma_sq_nu = (double*)ptr;  ptr += bytes_vec;
    b->lgamma_alpha    = (double*)ptr;  ptr += bytes_vec;
    b->lgamma_alpha_p5 = (double*)ptr;  ptr += bytes_vec;
    b->r               = (double*)ptr;  ptr += bytes_r;
    b->r_scratch       = (double*)ptr;  ptr += bytes_r;
