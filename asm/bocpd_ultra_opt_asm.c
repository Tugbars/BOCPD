...
static void fused_step_simd(bocpd_ultra_t *b, double x)
{
     ...
     * AVX2 assembly kernel for the inner loop. This provides ~8-15%
     * speedup over compiler-generated code.
     *=========================================================================*/
    {
        ...
        double r0 = args.r0_out;
        r_new[0] = r0;
        
        size_t last_valid = args.last_valid_out;
        ...
        double map_val = r0;
        size_t map_idx = 0;
        if (args.max_growth_out > map_val)
        {
            map_val = args.max_growth_out;
            map_idx = args.max_idx_out;
        }
                     ...
                     bocpd_prior_t prior, size_t max_run_length)
{
    ...
    size_t alloc_lin = alloc_scratch;  /* Same padding for linear buffers */
    ...
    /* Linear scratch buffers - also need padding for SIMD reads */
    b->lin_mu = aligned_alloc(64, alloc_lin);
    b->lin_C1 = aligned_alloc(64, alloc_lin);
    b->lin_C2 = aligned_alloc(64, alloc_lin);
    b->lin_inv_ssn = aligned_alloc(64, alloc_lin);
